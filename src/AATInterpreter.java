...
class AATInterpreter
{
	...
	int countStatements(AATStatement program)
	{
		if (program instanceof AATSequential)
		{
			...
			return countStatements(s.left()) + countStatements(s.right());
		}
		return 1;
	}
	...
	// convert tree to linear array of instructions
	int FlattenCode(AATStatement program, int IP)
	{
		...
		{
			labels.insert(((AATLabel) program).label().toString(), new Integer(
					IP));
		}
		return IP + 1;
	}
	...
	boolean EvalCallExp(AATCallExpression exp)
	{
		...
		for (i = exp.actuals().size() - 1; i >= 0; i--)
		{
			...
			// push args to function on stack
			SetMemory(SP, ACC);
			SP = SP - 4;
		}
	...
	boolean execute(AATStatement s)
	{
		...
		} else if (s instanceof AATConditionalJump)
		{
			...
			if (ACC != 0)
			{
				Integer jumpIndex = (Integer) labels
						.find(((AATConditionalJump) s).label().toString());
				if (jumpIndex == null)
				{
					System.out.println("ERROR: in AATConditionalJump");
					System.out.println("       Trying to jump to :"
							+ ((AATConditionalJump) s).label());
		...
		{
			Integer jumpIndex = (Integer) labels.find(((AATJump) s).label()
					.toString());
			if (jumpIndex == null)
			{
		...
		} else if (s instanceof AATMove)
		{
			...
			} else
			{
				...
				System.out
						.println("       LHS must be AATMemory or AATRegister!");
				return true;
			}
		} else if (s instanceof AATReturn)
		{
			...
			System.out
					.println("   It should not be possible to get here (AATReturn)!");
		...
		} else if (s instanceof AATSequential)
		{
			...
			System.out
					.println("   All AATSequentials should have been removed by flatten!");
			System.out.println("   Please report to Prof. Galles!");
			return true;
		} else
		{
			System.out.println("ERROR: malformed AAT, for statement: " + s);
			return true;
		}
		return false;
	}
	...
	boolean RunFunction()
	{
		while (true)
		{
			 ...
			 * System.out.println("\"" + code[IP].getSJava() + "\"");
			 * code[IP].Accept(pt);
			 */
			...
			if (code[IP - 1] instanceof AATReturn)
			{
				...
				// move program back to return address
				IP = ReturnAddr;
				return false;
			}
			...
			// printStack((STACKSIZE - 1) * 4, SP - 10);
		}
	}
	...
	public void printStack(int startIndex, int endIndex)
	{
		System.out.println("----------------------------");
		System.out.println("SP: " + SP);
		System.out.println("FP: " + FP);
		System.out.println("ReturnAddr: " + ReturnAddr);
		for (int i = startIndex; i >= endIndex; i -= 4)
		{
			System.out.print("#" + i + " [" + Memory[i / 4] + "]");
